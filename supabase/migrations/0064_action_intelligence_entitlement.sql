...
--
-- Follows the exact precedent 0062 (Planning) and 0063 (Decision)
-- established: a dedicated feature:* entitlement key plus its own
-- *_operations metered quota key, seeded together in one migration.
...
select p.id, 'feature:action_intelligence', 1, 'monthly'
from public.plans p
where p.code in ('pro', 'founding_pro')
on conflict (plan_id, quota_key) do nothing;

insert into public.plan_quotas (plan_id, quota_key, quota_limit, quota_period)
select p.id, 'action_intelligence_operations', 1000, 'monthly'
from public.plans p
where p.code in ('pro', 'founding_pro')
on conflict (plan_id, quota_key) do nothing;
