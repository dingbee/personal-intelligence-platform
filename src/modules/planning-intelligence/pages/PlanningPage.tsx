...
import type { InformationOrigin, Plan } from '@/modules/planning-intelligence/plan'
                    ...
                    <li key={m.id} className="rounded-control border border-[var(--color-border)] bg-[var(--surface-inset)] p-2.5 text-xs">
                      <div className="flex items-center justify-between gap-2">
                        <p className="font-medium text-[var(--color-ink)]">
                          {m.sequence}. {m.title}
                          {m.target && <span className="ml-2 font-normal text-[var(--color-ink-muted)]">({m.target})</span>}
                        </p>
                        <button
                          type="button"
                ...
                {[...plan.tasks]
                  ...
                  .map((t) => (
                    <li key={t.id} className="rounded-control border border-[var(--color-border)] bg-[var(--surface-inset)] p-2.5 text-xs">
                      ...
                      {(t.estimatedEffort || t.requiredResources.length > 0) && (
                        <p className="mt-0.5 text-[var(--color-ink-muted)]">
