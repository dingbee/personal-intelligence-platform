...
/**
 * UX-14.4.4 — `creationMethod` was written by every `createNote` call site
 * in UX-14.4.3, but nothing displayed it: this is the human-readable label
 * side, mirroring `ARTIFACT_KIND_DEFINITIONS`'s label convention, so
 * `NoteDetailPage.tsx` (its first reader) doesn't invent its own copy.
 */
export const ARTIFACT_CREATION_METHOD_LABELS: Record<ArtifactCreationMethod, string> = {
  user_created: 'Created manually',
  conversation_saved: 'Saved from a conversation',
  ai_generated: 'Generated by ARRIYIA',
  imported: 'Imported',
  derived: 'Merged from other notes',
}
