...
**Current state (as of UX-14.4.3).** NOVA can classify what an AI response *is* (`ArtifactKind` — Phase 1), export a deterministically-parsed spreadsheet to XLSX/CSV with formulas preserved and never evaluated (Path A — Phase 2), compile a structured, address-free `SpreadsheetSpecification` into the same output type Path A produces (Phase 3), and generate one from a natural-language request through the full validated pipeline (UX-14.4.2). UX-14.4.3 turned that generated output into a recognizable object rather than plain chat text: a generated spreadsheet now renders as a structured `ArtifactPreviewCard` (reusing `KnowledgeCard`) for the turn it appears in, every note-creation path now tags a consistent `creationMethod` (`user_created`/`conversation_saved`/`ai_generated`/`imported`/`derived`) into the same additive `generation_metadata` JSONB, and a `linkNoteToMemory` primitive exists on the same `knowledge_links` table "related knowledge" linkage already uses automatically. None of this changed persistence: `responseText` (what gets saved) is byte-identical to before, and a full unmocked regression test now proves generation → preview → save → note still retains `artifactKind`/`artifactData`/`creationMethod` correctly. Full records: `docs/ux-14.4-knowledge-artifact-architecture.md` (Phase 1), `docs/ux-14.4-phase2-spreadsheet-artifact-discovery.md` (Phase 2 + the UX-14.4.5 acceptance harness), `docs/ux-14.4-path-b-generation-discovery.md` and `docs/ux-14.4-spreadsheet-specification-discovery.md` (Path B design), `docs/ux-14.4.3-artifact-experience-layer-discovery.md` (Experience Layer discovery), `docs/ux-14-architecture-consolidation.md` (UX-14.4.1 through UX-14.4.3 implementation records).

**Target state (remaining, not yet built).** Chart/report/template artifact kinds have no generation capability yet — only spreadsheets do. `columnFormats`/column `type` hints still don't map to an actual Excel number format. A saved multi-sheet generated spec loses its sheet names and format hints on the Save-to-Notes markdown round-trip (both already-inert fields today, so a disclosed, zero-impact gap rather than a regression). Automatic "which memories relate to this artifact" matching is not built — `linkNoteToMemory` is a ready primitive with no caller yet, deliberately: no existing matching algorithm transfers to memory content, and building one, or plumbing memory ids through `retrieveMemoryContext`'s string-only return type, was judged speculative for this milestone. A "Reviewed" or "Improved" artifact lifecycle state has no home yet either — there is no persisted row before a Save happens, so those states are chat-history-only today (see the Experience Layer discovery §3).

**Risks.** The formula-injection risk the Path B architecture discovery identified is now actually mitigated, not just described: `validateFormulaSafety` runs on every compiled formula cell before `renderSpreadsheetArtifactMarkdown` ever sees it, and a rejection fails the whole generation closed (no partial output, nothing sanitized-and-passed). A second, unrelated risk was found and fixed during UX-14.4.2's own implementation, not predicted in advance: reusing `sheetToGrid.ts` (which imports `xlsx`) from the new eagerly-registered Workspace Action pulled the ~330KB `xlsx` library into the main bundle — caught by `verify:bundle` before commit, fixed by giving `cellAddressing.ts` a dependency-free `decodeCellAddress`, now covered by a permanent regression test. UX-14.4.3 checked for the same class of regression in its own new eagerly-loaded files (`ArtifactPreviewCard.tsx`/`MessageBubble.tsx`) and confirmed it didn't recur — the main bundle grew by ~1KB, exactly the size of the new code itself.

**Recommended sequence.** With generation, validation, formula safety, structured preview, and provenance all live, the next natural step is extending the same `Specification -> validate -> compile -> render` pattern to a second artifact kind (chart is the next-most-requested per the Roadmap), rather than deepening spreadsheet-only capability further. Sharing/collaboration (UX-14.5) is a separate, larger track blocked on a new authorization model — every table in the schema is still single-owner `auth.uid() = user_id` RLS, confirmed directly against every migration — and is not a prerequisite of, nor unlocked by, any of this artifact-metadata work.
