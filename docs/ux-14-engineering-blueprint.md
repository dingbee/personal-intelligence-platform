...
**Current state (as of Phase 3).** NOVA can classify what an AI response *is* (`ArtifactKind` — Phase 1), export a deterministically-parsed spreadsheet to XLSX/CSV with formulas preserved and never evaluated (Path A — Phase 2), and compile a structured, address-free `SpreadsheetSpecification` into the same output type Path A produces (Phase 3, this milestone) — verified byte-equivalent to Path A's output for the same logical table, not just independently correct. None of this involves an AI call yet: every piece shipped so far is deterministic classification, parsing, or compilation. Full records: `docs/ux-14.4-knowledge-artifact-architecture.md` (Phase 1), `docs/ux-14.4-phase2-spreadsheet-artifact-discovery.md` (Phase 2 + the UX-14.4.5 acceptance harness), `docs/ux-14.4-path-b-generation-discovery.md` and `docs/ux-14.4-spreadsheet-specification-discovery.md` (Path B design).

**Target state (remaining, not yet built).** An AI-facing capability (`generate-spreadsheet-artifact`) that produces a `SpreadsheetSpecification` from a natural-language request, routed through the existing Workspace Action Router and `runCapability`/`runWithFallback` governance (no new AI-runtime infrastructure needed — Path B's discovery found this already exists). A formula-content safety allowlist must land before any LLM-authored formula reaches the compiler. A propose-then-approve consent model is recommended over auto-save, mirroring UX-14.3.5's memory-capture precedent rather than Generate Briefing's.

**Risks.** The formula-injection finding from the Path B architecture discovery is the one genuinely new risk class in this track — an LLM given direct authorship of spreadsheet formulas is a different, larger attack surface than Path A's incidental formulas parsed from ordinary chat prose. Not yet mitigated because no formula generation exists yet; must be resolved before, not after, Path B ships.

**Recommended sequence.** Phase 3's compiler is the prerequisite Path B needs, not an alternative to it. Next: extract the flagged `cellAddressing.ts` (zero-risk), then Path B's capability/prompt registration with the safety allowlist built in from the start, not bolted on after.
