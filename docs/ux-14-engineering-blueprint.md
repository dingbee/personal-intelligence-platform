...
**Current state (as of UX-14.4.1).** NOVA can classify what an AI response *is* (`ArtifactKind` — Phase 1), export a deterministically-parsed spreadsheet to XLSX/CSV with formulas preserved and never evaluated (Path A — Phase 2), and compile a structured, address-free `SpreadsheetSpecification` into the same output type Path A produces (Phase 3) — verified byte-equivalent to Path A's output for the same logical table, not just independently correct. UX-14.4.1 hardened this foundation ahead of Path B: the previously-duplicated `columnLetter` now lives in one shared `cellAddressing.ts`, and a formula-content safety allowlist (`validateFormulaSafety.ts` — cell-reference/five-function allowlist, rejects `WEBSERVICE`/`HYPERLINK`/`IMPORTXML`/DDE-style payloads) exists as a standalone, fully-tested module. None of this involves an AI call yet: every piece shipped so far is deterministic classification, parsing, compilation, or validation. Full records: `docs/ux-14.4-knowledge-artifact-architecture.md` (Phase 1), `docs/ux-14.4-phase2-spreadsheet-artifact-discovery.md` (Phase 2 + the UX-14.4.5 acceptance harness), `docs/ux-14.4-path-b-generation-discovery.md` and `docs/ux-14.4-spreadsheet-specification-discovery.md` (Path B design), `docs/ux-14-architecture-consolidation.md` (UX-14.4.1 implementation record).

**Target state (remaining, not yet built).** An AI-facing capability (`generate-spreadsheet-artifact`) that produces a `SpreadsheetSpecification` from a natural-language request, routed through the existing Workspace Action Router and `runCapability`/`runWithFallback` governance (no new AI-runtime infrastructure needed — Path B's discovery found this already exists). `validateFormulaSafety` needs to actually be wired into that generation path before any LLM-authored formula reaches the compiler — it exists now but is not yet called from anywhere. A propose-then-approve consent model is recommended over auto-save, mirroring UX-14.3.5's memory-capture precedent rather than Generate Briefing's.

**Risks.** The formula-injection finding from the Path B architecture discovery is the one genuinely new risk class in this track — an LLM given direct authorship of spreadsheet formulas is a different, larger attack surface than Path A's incidental formulas parsed from ordinary chat prose. The allowlist that mitigates it now exists (UX-14.4.1) but is inert until Path B's generation path actually calls it — building the module is not the same as closing the risk.

**Recommended sequence.** Phase 3's compiler and UX-14.4.1's hardening are the prerequisites Path B needs, not alternatives to it. Next: Path B's capability/prompt registration, calling `validateFormulaSafety` on every generated formula before it reaches the compiler — not bolted on after.
