...
Settings is where you control how ARRIYIA operates rather than what it knows — which AI providers it uses, how workspaces are managed, and what ARRIYIA remembers about you as a person, not just as an account.
...
- **Memory management** — review and manage what ARRIYIA has learned or been told about you (explicit facts, learned preferences, conversation-derived context)
...
- ARRIYIA has learned you prefer concise answers over long ones from repeated conversation patterns — that's visible and manageable from Memory, not a silent, unaccountable adjustment.
...
2. **Review memory periodically**: check what ARRIYIA has learned about you occasionally, especially if a Chat response's personalization feels off — the "how ARRIYIA uses this" explanation on each memory item shows exactly what it's influencing.
...
- Not realizing memory categories are distinct — explicit facts (things you directly told ARRIYIA), learned preferences (inferred from behavior), and conversation memory (context from past chats) are managed and weighted differently.
