...
- **`detectMemoryCandidates` has no live caller** — grep across `src/` found it invoked only from its own test file. `MemoryApprovalPanel`'s candidate queue is always empty in production today, so this milestone's persistence fix has no observable effect until a future milestone wires detection into a real chat surface. Discovered during Phase 1 discovery, not assumed; recorded as the first item in UX-14.3's remaining roadmap in `docs/ux-14-architecture-consolidation.md`.
